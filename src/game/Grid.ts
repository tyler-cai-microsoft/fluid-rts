...
import { getUniqueUnitId } from "./SerializableUnit";
...
export class Grid {
	...
	unitsMap = new Map<string, IUnit>();
	constructor(
		private readonly width: number,
		private readonly height: number,
	) {
		console.log(
			`Width: ${width}, Height: ${height}, Widths: ${width / gridSize}, Heights: ${
				height / gridSize
			}`,
		);
		this.grid = [];
	...
	public updateUnitOnGrid(unit: IUnit) {
		...
		this.unitsMap.set(getUniqueUnitId(unit), unit);
	}

	private deleteUnitOnGrid(unit: IUnit) {
	...
	public killUnit(unit: IUnit) {
		this.deleteUnitOnGrid(unit);
		this.unitsMap.delete(getUniqueUnitId(unit));
	}
	...
	public getUnitsInRadius(x: number, y: number, radius: number): IUnit[] {
		...
		for (let i = gridPoint.x - radius; i <= gridPoint.x + radius; i += gridSize) {
			for (let j = gridPoint.y - radius; j <= gridPoint.y + radius; j += gridSize) {
				...
				for (const unit of gridPoint.units) {
					if (unit.health <= 0) {
						this.deleteUnitOnGrid(unit);
					}
				}
